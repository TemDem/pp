(() => {
    ...
    function updateClasses({el, slides, activeIndex}) {
        ...
        const bulletNext = document.querySelectorAll(".swiper-pagination-bullet-active-next-next-next");
        const bulletPrev = document.querySelectorAll(".swiper-pagination-bullet-active-prev-prev-prev");
        if (bulletNext.length) bulletNext.forEach((el => {
            el.classList.remove("swiper-pagination-bullet-active-next-next-next");
        }));
        if (bulletPrev.length) bulletPrev.forEach((el => {
            el.classList.remove("swiper-pagination-bullet-active-prev-prev-prev");
        }));
        bullets.forEach(((el, i) => {
            if (el.classList.contains("swiper-pagination-bullet-active")) {
                if (bullets[i + 3]) bullets[i + 3].classList.add("swiper-pagination-bullet-active-next-next-next");
                if (bullets[i - 3]) bullets[i - 3].classList.add("swiper-pagination-bullet-active-prev-prev-prev");
            }
        }));
        bullets[bullets.length - 2].classList.add("_last-prev");
        if (screenWidth > 992) {
            const opa = document.querySelectorAll("._opa");
            opa.forEach((el => {
                el.classList.remove("_opa");
            }));
            if (slides.length > activeIndex + 4) slides[activeIndex + 4].classList.add("_opa");
        } else if (screenWidth > 768) {
            const opa = document.querySelectorAll("._opa");
            opa.forEach((el => {
                el.classList.remove("_opa");
            }));
            if (slides.length > activeIndex + 3) slides[activeIndex + 3].classList.add("_opa");
        } else if (screenWidth > 480) {
            const opa = document.querySelectorAll("._opa");
            opa.forEach((el => {
                el.classList.remove("_opa");
            }));
            if (slides.length > activeIndex + 2) slides[activeIndex + 2].classList.add("_opa");
        } else {
            const opa = document.querySelectorAll("._opa");
            opa.forEach((el => {
                el.classList.remove("_opa");
            }));
            if (slides.length > activeIndex + 1) slides[activeIndex + 1].classList.add("_opa");
        }
    }
